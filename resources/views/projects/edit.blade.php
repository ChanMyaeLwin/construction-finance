        ...
        <form method="POST" action="{{ route('projects.update', $project) }}" class="space-y-3">
            ...
            <div>
                ...
                <select name="project_type_id" class="mt-1 w-full rounded-xl border-gray-300">
                    ...
                    @foreach($projectTypes as $t)
                        <option value="{{ $t->id }}" @selected(old('project_type_id', $project->project_type_id)==$t->id)>{{ $t->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="mt-1 w-full rounded-xl border-gray-300 focus:ring-indigo-500">{{ old('description', $project->description) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ old('start_date', optional($project->start_date)->format('Y-m-d')) }}" required
                           class="mt-1 w-full rounded-xl border-gray-300 focus:ring-indigo-500" />
                </div>
                <div>
                    <label class="block text-sm font-medium">Expected End Date</label>
                    <input type="date" name="expected_end_date" value="{{ old('expected_end_date', optional($project->expected_end_date)->format('Y-m-d')) }}"
                           class="mt-1 w-full rounded-xl border-gray-300 focus:ring-indigo-500" />
                </div>
            </div>

            <div class="pt-2">
                <button class="w-full sm:w-auto px-4 py-2 rounded-xl bg-indigo-600 text-white font-medium hover:bg-indigo-700">Update</button>
                <a href="{{ route('projects.show', $project) }}" class="ml-2 text-gray-600">Cancel</a>
            </div>
        </form>
    </div>
</x-app-layout>